...
def draw_score(surface, score):
    ...
    surface.blit(label, (top_left_x + GAME_WIDTH + 50, top_left_y + 50))
...
def draw_text_top_middle(text, size, color, surface):
    font = pygame.font.SysFont('comicsans', size, bold=True)
    label = font.render(text, 1, color)
    surface.blit(label, (top_left_x, top_left_y))

def draw_grid(surface, row, col):
    sx = top_left_x
    sy = top_left_y
    for i in range(row):
        pygame.draw.line(surface, (128,128,128), (sx, sy+ i*30), (sx + GAME_WIDTH, sy + i * 30))  # horizontal lines
        for j in range(col):
            pygame.draw.line(surface, (128,128,128), (sx + j * 30, sy), (sx + j * 30, sy + GAME_HEIGHT))  # vertical lines
...
def main_menu():
    ...
    while run:
        ...
        draw_text_top_middle('High Score: ' + str(high_score), 40, (255, 255, 255), win)
        pygame.display.update()
